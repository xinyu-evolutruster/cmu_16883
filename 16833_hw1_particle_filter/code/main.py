...
def init_particles_freespace(num_particles, occupancy_map):
    ...
    while len(x0_vals) < num_particles:
        ...
        for i in range(len(x_map)):
            if occupancy_map[y_map[i], x_map[i]] == 0:
...
if __name__ == '__main__':
    ...
    for time_idx, line in enumerate(logfile):
        ...
        for m in range(0, num_particles):
            ...
            if (meas_type == "L"):
                z_t = ranges
                w_t, z_stars = sensor_model.beam_range_finder_model(z_t, x_t1)
                ...
                #     resolution = 10
                #     for i in range(180):
                #         theta = x_t1[2] + i * np.pi / 180 - np.pi / 2
                #         dist = z_stars[i]
                #         x = int(x_t1[0] / resolution)
                #         y = int(x_t1[1] / resolution)
                #         # print("dist = ", dist)
                #         dx = dist * np.cos(theta)
                #         dy = dist * np.sin(theta)
                #         # print("dx = {}, dy = {}".format(dx, dy))
                #         new_x = int(x_t1[0] + dx) // resolution
                #         new_y = int(x_t1[1] + dy) // resolution
                #         # print("x = {}, new_x = {}, y = {}, new_y = {}".format(
                #         #     x, y, new_x, new_y))
                #         scan = plt.plot([x, new_x], [y, new_y],
                #                         c='red', linewidth=0.1)
                #     plt.savefig('test_zstar.png')
                #     for s in scan:
                #         s.remove()
